...
def solve(data):
    N = [(ix, int(el)*811589153) for (ix, el) in enumerate(data.splitlines())]
    L = len(N)
    d = N.copy()
    for i in range(10):
        for n in N:
...
def test_sample():
    assert solve(import_data(True)) == 1623178306


def test_real():
    assert solve(import_data(False)) == 3760092545849
